    ...
    module panel_geometry() {
        ...
        difference(){
            ...
            if(gaps == true){
                ...
                global_chamfer_cube(dimensions = [panel_width, panel_width, thickness], chamfer_depth = chamfer_depth );
            }
            else{
                translate([0,0,gapless_offset_z])
                global_chamfer_cube(dimensions = [hub_spacing, hub_spacing, thickness], chamfer_depth = chamfer_depth);
            }

            // cutting hub proxies
            translate(chamfer_cube_to_center_vector)
            global_chamfer_cube(dimensions= chamfer_cube_dims, chamfer_depth=hub_octagon_triangle_leg);
            ...
            global_chamfering_pyramid(base = hub_octagon_side_length + 6 * chamfer_depth);
