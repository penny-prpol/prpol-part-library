...
union(){
    difference(){
        spur_gear (module_size=1, tooth_number=size1*10, width=thickness1, bore=axis_bore_diameter, pressure_angle=25, helix_angle=0, optimized=false);
        if(size1 > 3){
            difference(){
    ...
    translate([0,0,10]){
        spur_gear (module_size=1, tooth_number=10, width=thickness1, bore=axis_bore_diameter, pressure_angle=25, helix_angle=0, optimized=false);
    }
}
}
