...
3. **Start OpenSCAD, choose Open, navigate to your copy of this repository on your machine, and select my-first-project.scad.** You can think of my-first-project.scad as your workspace for generating parts. It has a special include statement at the top of the file that links it to all the files that contain the PRPOL OpenSCAD modules. You will also find a collection of usage examples as a comment in my-first-project.scad.
4. **Below the last include statement, type bin(); then find and press Render below the 3D viewport or hit F6.** Congratulations, you've just generated your first PRPOL part. Next try typing bin([2,3,4]); instead of bin(); and press Render again. Congratulations! You've just customized your first PRPOL part! The bin() module is just one of many modules available for generating PRPOL parts.
...
### Truss
Whereas Block and Plate parts are joined together with M3 screws and nuts, Truss parts lock together without the need for fasteners. Truss parts include Hubs and Struts. The compact Hubs pack a lot of connective potential in a small package: A Strut can be inserted into any one of 18 connection pockets in a Hub, then the Strut is rotated 90 degrees to lock it in place. Hubs can also be printed with six M3 machine screws captive inside to allow other kinds of structural parts to be screwed directly to a Truss assembly. Truss parts are the most material and time-efficient way to make larger structures. When assembling Hubs and Struts, keep in mind that all the Hubs need to be oriented the same way. The default Strut lengths are set so that the **centers of hubs are exactly 10 cm to each adjacent hub center.** The shorter Leg Strut connects adjacent hubs in a 3D cubic grid pattern, and the longer Hyp (hypotenuse) Strut connects diagonal Hubs to make the truss rigid. It is perfectly acceptable for an individual truss connection to be a bit wobbly, because as long as the truss structure has both Leg and Hyp struts, the structure as a whole will still be rigid. 

## Transition Parts

Use transition parts to attach off-the-shelf components like motors, circuit boards, and battery holders onto PRPOL structural parts. 

### Universal Transition Parts

Any off-the-shelf component that has mounting holes that don't line up with a 10mm grid can be attached with slot plate or slot brackets.

A slot plate is a flat rectangular part with a grid of slots instead of circular
holes. Many off-the-shelf components can attach to slot plate directly.

A slot bracket is a small part that acts like an adjustable anchor point to bolt an off-the-shelf component to. 

### Motor Holders

Prpol currently has motor holders for mini servos, TT motors, and generic hobby-sized DC motors. Planned additions include all common servo motor sizes, as well as common stepper motors. 

## Motion Parts

Motion parts currently include wheels and spur gears. Planned additions include worm gears, rack gears, chain and sprocket, tank tracks, and parametric end affectors like robot grippers.
